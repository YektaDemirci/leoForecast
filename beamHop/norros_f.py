# Norros / Gripenberg-Norros fBm predictor.
...
from linearP_f import fgn_autocov, linearp_weights
...
fgn_weights = linearp_weights


def select_H(Z, T, horizon=1, grid=None, kernel="fgn"):
    if grid is None:
        grid = np.arange(0.55, 0.9951, 0.005)
    grid = np.asarray(grid, dtype=float)
    win, truth = design(Z, T, horizon)
    if len(truth) < 2:
        raise ValueError(f"need > T + horizon = {T + horizon} samples to select "
                         f"H, got {len(Z)}")
    denom = np.var(truth)
    build = ((lambda H: fgn_weights(T, H, horizon)) if kernel == "fgn"
             else (lambda H: norros_weights(T, H, horizon=float(horizon))))
    curve = np.array([np.mean((truth - win @ build(H)) ** 2) / denom
                      for H in grid])
    return float(grid[np.argmin(curve)]), grid, curve
...
def main():
    import pandas as pd
    from cell_buffer_sim import analyze_traffic_model   # local Whittle H
    ...
    m, a, H = analyze_traffic_model(train, args.dt)
