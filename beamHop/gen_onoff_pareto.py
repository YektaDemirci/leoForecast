...
def _bin_on_intervals(starts, ends, n_bins, dt, occ, diff):
    """Accumulate the exact per-bin ON-time of intervals [start, end) into occ.

    Each interval spreads its length over the bins it overlaps: partial
    head/tail contributions via scatter_add, fully covered middle bins via a
    difference array + cumsum. Everything is O(#intervals) on the GPU.
...
def local_whittle_H(x, m=None):
    ...
    periodogram = (np.abs(np.fft.fft(x - x.mean())) ** 2 / (2 * np.pi * n))[1:m + 1]
    lam = 2 * np.pi * np.arange(1, m + 1) / n

    def objective(H):
        d = H - 0.5
        return (np.log(np.mean(lam ** (2 * d) * periodogram))
                - 2 * d * np.mean(np.log(lam)))

    return minimize_scalar(objective, bounds=(0.5001, 0.9999),
                           method="bounded").x
...
if __name__ == "__main__":
    ...
    H_hat = local_whittle_H(rate_np)
    print(f"Hurst:     estimated {H_hat:.3f}  theory {H_theory:.3f} "
          f"(local Whittle, m = n**0.65, se ~ {1/(2*(len(rate_np)**0.65)**0.5):.3f})")
